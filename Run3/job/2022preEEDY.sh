...
KEEPAOD=${KEEPAOD%/}
[ -n "$KEEPAOD" ] && KEEPAOD="$KEEPAOD/$OUTTAG"
TAG="DY2022preEE"

# All conditions below are taken from the central production chain on McM:
#   GEN-SIM     CMSSW_12_4_11_patch3 (124X_mcRun3_2022_realistic_v12)
#   DIGI+HLT    CMSSW_12_4_11_patch3 (124X_mcRun3_2022_realistic_v12), HLT:2022v12
#   MINIAOD     CMSSW_13_0_13 (130X_mcRun3_2022_realistic_v5)
#   NANOAOD     CMSSW_13_0_13 (130X_mcRun3_2022_realistic_v5)
...
use_release CMSSW_12_4_11_patch3 el8_amd64_gcc10
...
cmsDriver.py Configuration/GenProduction/python/$Fragment_filename \
    ...
    --step LHE,GEN,SIM --geometry DB:Extended --era Run3 \
...
cmsDriver.py --python_filename $TAG"__DIGIPREMIX__cfg_"$NJOB".py" --eventcontent PREMIXRAW \
    ...
    --conditions 124X_mcRun3_2022_realistic_v12 --step DIGI,DATAMIX,L1,DIGI2RAW,HLT:2022v12 \
    --procModifiers premix_stage2,siPixelQualityRawToDigi --geometry DB:Extended \
    --filein file:$Input_filename --datamix PreMix --era Run3 \
...
cmsDriver.py --python_filename $TAG"__AOD__cfg_"$NJOB".py" --eventcontent AODSIM \
    ...
    --geometry DB:Extended --filein file:$Input_filename --era Run3 \
    --no_exec --mc -n -1 --nThreads 4
cmsRun $TAG"__AOD__cfg_"$NJOB".py"

if [ -n "$KEEPAOD" ] && [ -e $Output_filename ]
then
    AODOUT=$TAG"__job-"$NJOB"_"$OUTTAG"__AOD.root"
    cp $Output_filename $AODOUT
    K_REDIR=$(echo $KEEPAOD | sed 's@\(root://[^/]*\)//.*@\1@')
    K_PATH=$(echo $KEEPAOD | sed 's@root://[^/]*/@@')
    xrdfs $K_REDIR mkdir -p $K_PATH
    for attempt in 1 2 3
    do
        xrdcp -f $AODOUT $KEEPAOD/. && break
        echo "AOD stageout attempt $attempt failed, retrying in 60 s"
        sleep 60
    done
    rm -f $AODOUT
fi
...
echo ---------------------------MINIAOD-------------------------
use_release CMSSW_13_0_13 el8_amd64_gcc11
...
cmsDriver.py --python_filename $TAG"__MINIAOD__cfg_"$NJOB".py" --eventcontent MINIAODSIM \
    ...
    --conditions 130X_mcRun3_2022_realistic_v5 --step PAT --geometry DB:Extended \
    --filein file:$Input_filename --era Run3,run3_miniAOD_12X \
    --no_exec --mc -n -1 --nThreads 4
...
cmsDriver.py --python_filename $TAG"__NANOAOD__cfg_"$NJOB".py" --eventcontent NANOAODSIM \
    ...
    --conditions 130X_mcRun3_2022_realistic_v5 --step NANO --scenario pp \
    --filein file:$Input_filename --era Run3 \
    --no_exec --mc -n -1 --nThreads 4
...
rm -rf CMSSW_12_4_11_patch3 CMSSW_13_0_13
